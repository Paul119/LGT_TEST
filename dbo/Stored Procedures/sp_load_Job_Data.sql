CREATE    PROCEDURE [dbo].[sp_load_Job_Data]  
...
BEGIN TRY  
  ...
  DECLARE @sql NVARCHAR(MAX) = 'SELECT * INTO _tb_employee_Job'+'_'+convert(varchar, getdate(), 112)	 + replace(convert(varchar, getdate(),108),':','')+' FROM _tb_employee_Job'

  EXEC sys.sp_executesql @sql

  TRUNCATE TABLE _tb_employee_Job
  INSERT INTO _tb_employee_Job (IdPayee,PersonnelNumber,EffectiveDate,EndDate,JobCode,id_user,CreatedDate,ModificationDate,ParentId)
  SELECT IdPayee
		,PersonnelNumber
