CREATE   PROCEDURE [dbo].[sp_grid_save_post_Title_Data_actions]  
...
DECLARE @ActionId INT = ( SELECT ActionId FROM _stg_xls_TitleData_Action WHERE Id = @primaryKey)
...
BEGIN TRY		
	...
	BEGIN
				...
				TRUNCATE TABLE _stg_xls_TitleData 
				TRUNCATE TABLE _stg_xls_TitleData_Error

				--Reset the action
				UPDATE _stg_xls_TitleData_Action
				SET ActionId = 0
	...
	BEGIN
				...
				EXEC sp_load_Title_Data_information_import_log @qc_audit_key, @UserId

				EXEC sp_load_Title_Data @qc_audit_key, @UserId
