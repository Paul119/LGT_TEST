...
CREATE PROCEDURE [dbo].[sp_grid_delete_post_master_emp_org]
...
--DECLARE @startdate date = (SELECT EffectiveDate FROM _tb_employee_organization WHERE employeeOrganizationId = @primaryKey)
--DECLARE @MaximunEndDate DATE =  (SELECT MAX(EndDate) FROM _tb_employee_organization WHERE idPayee =  @idpayee AND employeeOrganizationId != @primaryKey) 
...
BEGIN TRY
	...
	--UPDATE _tb_employee_organization
	--SET PersonnelNumber = @employeeGUID
	--WHERE employeeOrganizationId = @primaryKey
	...
	FROM _tb_employee_organization
	--WHERE IdPayee = @IdPayee
	)

	UPDATE f
	SET f.EndDate = c.EndDate
	FROM _tb_employee_organization f
