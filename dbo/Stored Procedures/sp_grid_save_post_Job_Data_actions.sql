CREATE   PROCEDURE [dbo].[sp_grid_save_post_Job_Data_actions]  
...
DECLARE @ActionId INT = ( SELECT ActionId FROM _stg_xls_JobData_Action WHERE Id = @primaryKey)
...
BEGIN TRY		
	...
	BEGIN
				...
				TRUNCATE TABLE _stg_xls_JobData 
				TRUNCATE TABLE _stg_xls_JobData_Error

				--Reset the action
				UPDATE _stg_xls_JobData_Action
				SET ActionId = 0
	...
	BEGIN
				...
				EXEC sp_load_Job_Data_information_import_log @qc_audit_key, @UserId

				EXEC sp_load_Job_Data @qc_audit_key, @UserId
