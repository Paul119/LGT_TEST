...
CREATE   PROCEDURE [dbo].[sp_grid_save_post_Compensation_Data_actions]  
...
DECLARE @ActionId INT = ( SELECT ActionId FROM _stg_xls_CompensationData_Action WHERE Id = @primaryKey)
...
BEGIN TRY		
	...
	BEGIN
				...
				TRUNCATE TABLE _stg_xls_CompensationData
				TRUNCATE TABLE _stg_xls_CompensationData_Error

				--Reset the action
				UPDATE _stg_xls_CompensationData_Action
				SET ActionId = 0
	...
	BEGIN
				...
				EXEC sp_load_Compensation_Data_information_import_log @qc_audit_key, @UserId

				EXEC sp_load_Compensation_Data @qc_audit_key, @UserId
	...
	ELSE 
	BEGIN
				SET @Txt = 'Action requested: "N/A"'+CAST(@primaryKey AS NVARCHAR(2))
				EXEC sp_audit_log @Category, @Process, @SubProcess, @StartDate, @Txt, @qc_audit_key, @UserId
