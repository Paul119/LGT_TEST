...
CREATE PROCEDURE [dbo].[sp_grid_delete_post_master_emp_job]
...
--DECLARE @idpayee INT = (SELECT idpayee FROM _tb_employee_Job tej WHERE tej.employeeJobId = @primaryKey)


--DECLARE @employeeGUID NVARCHAR(50) = (SELECT codepayee FROM py_Payee pp WHERE pp.idPayee = @idpayee)
--DECLARE @startdate date = (SELECT EffectiveDate FROM _tb_employee_Job WHERE employeeJobId = @primaryKey)
--DECLARE @MaximunEndDate DATE =  (SELECT MAX(EndDate) FROM _tb_employee_Job WHERE idPayee =  @idpayee AND employeeJobId != @primaryKey) 
...
BEGIN TRY
	...
	--UPDATE _tb_employee_Job
	--SET PersonnelNumber = @employeeGUID
	--WHERE employeeJobId = @primaryKey
	...
	FROM _tb_employee_Job
	--WHERE IdPayee = @IdPayee
	)

	UPDATE f
	SET f.EndDate = c.EndDate
	FROM _tb_employee_Job f
