...
CREATE   PROCEDURE [dbo].[sp_grid_save_post_employee_information_import_action]
...
DECLARE @ActionId INT =( SELECT ActionId FROM _stg_xls_EmployeeData_Action WHERE Id = @primaryKey)
...
BEGIN TRY		
	...
	BEGIN
				...
				TRUNCATE TABLE _stg_xls_EmployeeData
				TRUNCATE TABLE _stg_xls_EmployeeData_Error

				--Reset the action
				UPDATE _stg_xls_EmployeeData_Action
				SET ActionId = 0
	...
	BEGIN
				...
				EXEC sp_load_employee_import @qc_audit_key, @UserId
