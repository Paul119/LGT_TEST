...
CREATE PROCEDURE [dbo].[sp_grid_add_pre_master_emp_fte]
...
--SELECT * FROM py_Payee pp WHERE firstname = 'Jef'
	...
	BEGIN 
		SET @ResultStatus = 0;
		SELECT @ResultMessage = 'Start and end dates must be specified.'


	END
	--2. Start date can not stacked
	IF exists (SELECT 1 FROM  _tb_employee_fte WHERE idPayee = @idpayee AND @startdate = EffectiveDate  ) 
