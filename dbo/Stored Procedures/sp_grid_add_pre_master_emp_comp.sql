...
CREATE PROCEDURE [dbo].[sp_grid_add_pre_master_emp_comp]
...
--SET @startdate = (SELECT CONVERT(varchar, @startdate, 1))
	DECLARE @enddate date = 
		...
		DECLARE @payrolltype NVARCHAR(10) = 
		(SELECT Column_Value
			FROM grid_data_temp_values
			WHERE Unique_Key = @UniqueKey
			AND PK_Id = @primaryKey
			AND Column_Name = 'PayrollType')
...
--SELECT * FROM py_Payee pp WHERE firstname = 'Jef'
	...
	BEGIN 
		SET @ResultStatus = 0;
		SELECT @ResultMessage = 'Start date must be specified.'


	END
	-----2. Start date can not stacked
	IF exists (SELECT 1 FROM  _tb_employee_compensation WHERE idPayee = @idpayee AND PayrollType = @payrolltype AND @startdate = AwardDate ) 
