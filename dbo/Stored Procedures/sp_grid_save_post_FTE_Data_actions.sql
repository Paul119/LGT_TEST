CREATE PROCEDURE dbo.sp_grid_save_post_FTE_Data_actions  
...
DECLARE @ActionId INT = ( SELECT ActionId FROM _stg_xls_FTEData_Action WHERE Id = @primaryKey)
...
BEGIN TRY		
	...
	BEGIN
				...
				TRUNCATE TABLE _stg_xls_FTEData 
				TRUNCATE TABLE _stg_xls_FTEData_Error

				--Reset the action
				UPDATE _stg_xls_FTEData_Action
				SET ActionId = 0
	...
	BEGIN
				...
				EXEC sp_load_FTE_Data_information_import_log @qc_audit_key, @UserId
				EXEC sp_audit_log @Category, @Process, @SubProcess, @StartDate, @Txt, @qc_audit_key, @UserId

				EXEC sp_load_FTE_Data @qc_audit_key, @UserId
