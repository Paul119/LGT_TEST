CREATE PROCEDURE dbo.sp_load_FTE_Data_information_import_log  
...
BEGIN TRY  
  ...
  FTE = REPLACE(FTE,',','.')
   ...
   FROM _stg_xls_FTEData AS sourceTable  
   ...
   FROM _stg_xls_FTEData AS sourceTable  
   WHERE 1=1  
   AND NULLIF(sourceTable.FTE,'') IS NULL  
  ...
  ---------------------- FTE > 1 ----------------------  
   ...
   FROM _stg_xls_FTEData AS sourceTable  
   WHERE 1=1  
   AND CAST(sourceTable.FTE AS Float) > 1  
   
  --Delete previous logs  
  TRUNCATE TABLE _stg_xls_FTEData_Error

  INSERT INTO @errorfound (ID)
   SELECT stgFTEDataId FROM _stg_xls_FTEData
   ...
   INNER JOIN _stg_xls_FTEData sourcetable
   ON errorfound.ID = sourcetable.stgFTEDataId
   where 1=1
   ...
   SET @position = 5
   UPDATE errorfound
   SET Error = CONCAT(Error,'|','FTE null')
   FROM @errorfound AS errorfound
   INNER JOIN _stg_xls_FTEData sourcetable
   ON errorfound.ID = sourcetable.stgFTEDataId
   where 1=1
   ...
   --SELECT RIGHT(LEFT(QcStatusCode, 150-6),1),* FROM _stg_xls_FTEData
   --where LEFT(QcStatusCode, 150 - 6 - 1) + '1' + LEFT(RIGHT(QcStatusCode, 6),6 - 1) + '1'  = 1


  -- Insert errors into log table  
  INSERT INTO _stg_xls_FTEData_Error (PersonnelNumber,error,EffectiveDate ,FTE, ParentId)  
  SELECT   
   ...
   ON sourcetable.stgFTEDataId = ef.ID
